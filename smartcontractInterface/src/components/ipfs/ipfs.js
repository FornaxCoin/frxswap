...
const ipfsGateway = 'http://172.104.178.250:8080/ipfs/'
const ipfs = create('http://172.104.178.250:5001')
...
export default function IPFSForm() {
  ...
  async function uploadToIpfs() {
    ...
    console.log('writing article to IPFS now:')
    await ipfs.files.write('/example/article.txt',article,{ create: true })
    console.log('getting article CID')
    const { cid } = await ipfs.add(article)
    console.log('CID of Article', cid)
